<template>
  <div class="root" transition="fade">
    <project
    :work="work"
    :description="'A series of arranged “portrait” pictures.'"
...
module.exports = {
  ...
  route: {
    activate: function () {
      if (app.getRouter().app.$data.redirect === true){
        var _this = this;
        app.getRouter().app.$data.nextRoute = _this.thepictures.route;
  ...
  ready: function() {
    if (app.getRouter().app.$data.redirect === false){
      ...
      window.scrollTo(0,0);
      var _this = this;
      graphics = new Graphics(_this, _this.thepictures.titlePicture);
